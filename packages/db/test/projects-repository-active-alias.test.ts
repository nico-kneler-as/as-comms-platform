...
describe("settings.projects.setActive — alias requirement", () => {
  ...
  it("listAllProjectAliases reads the project_aliases table (email-alias source of truth for D-049)", async () => {
    const context = await createTestStage1Context();
    const now = new Date("2026-06-01T12:00:00Z");
    try {
      // Two project_dimensions rows — these are NOT the source of truth
      // for the bubble-side renderer. Their `project_alias` is a "short
      // internal project name" / display label, not an email.
      await context.repositories.projectDimensions.upsert({
        projectId: "project_pnw",
        projectName: "PNW Biodiversity",
        projectAlias: "PNW Biodiversity",
        source: "salesforce",
      });
      await context.repositories.projectDimensions.upsert({
        projectId: "project_past",
        projectName: "Past Orcas",
        projectAlias: "Past Orcas",
        source: "salesforce",
      });

      // project_aliases is the source of truth — the admin-managed table
      // of project inbox email addresses. listAllProjectAliases should
      // surface every row here, lowercased + trimmed + deduped.
