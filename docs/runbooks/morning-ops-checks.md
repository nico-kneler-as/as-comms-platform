...
- **Code paths:**
  - [email.ts](../../apps/worker/src/jobs/integration-health/email.ts) — alert email; subject format is `[AS Comms] <service> integration degraded — <status>`; 1-hour cooldown per service
  - [settings-records.ts](../../packages/contracts/src/settings-records.ts) — status values that trigger action
