...
- email timeline bubbles render right-side iff the sender email is a project inbox alias — the union of (a) any value in `project_dimensions.project_alias` (active or inactive) and (b) any value in `project_dimensions.previous_aliases` (rename-history array). All other senders, including AS staff sending from personal `@adventurescientists.org` addresses, render left-side. SMS messages, the inbox row tag, and the inbox row bump-to-`New` rule are unchanged (D-049)
