...
- email timeline bubbles render right-side iff the sender email is a project inbox alias — any value in `project_aliases.alias` (the admin-managed email-alias table). All other senders, including AS staff sending from personal `@adventurescientists.org` addresses, render left-side. SMS messages, the inbox row tag, and the inbox row bump-to-`New` rule are unchanged (D-049)
